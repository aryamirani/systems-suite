...
uartwrite(char buf[], int n)
{
  ...
  while(i < n){ 
    while(tx_busy != 0){
      ...
      // to set tx_busy to 0.
#ifdef FCFS
      // In FCFS, user processes (pid > 2) don't sleep - they busy wait
      // to maintain pure FCFS behavior where processes never give up CPU
      struct proc *p = myproc();
      if(p && p->pid > 2) {
        // Busy wait for user processes in FCFS
        release(&tx_lock);
        acquire(&tx_lock);
      } else {
